  ...
  ";
   ...
   for($t=0;$t<$no;$t++) {
    ...
    for($i=1;$i<$days_to_display;$i++) {
      ...
      echo "<tr class='$rowtype'><td>$dayname:</td><td>  $search_day / $search_month / $search_year:</td><td>";
        ...
        if(array_key_exists($search_month,$schedule[$search_year])) {
          if(array_key_exists($search_day,$schedule[$search_year][$search_month])) {
            ...
            for($j=0;$j<$classes_for_today;$j++) {
                #Start the table for the class details - DAY classes
                if (j==0){
                echo "<table class='calclasses'><tr><td width=40> $j Day:</td> ";
                }
                #If there's a night class - start the NIGHT sub table here
              if($j>0) echo "<br /><table class='calclasses'><tr><td width=40>Night: </td>";
              ...
              if($bookings==18) {
                echo "<td width = 150><s>".$class."</s></td><td><span style='color:red'>FULL!</span></td><td><a href='waitlist.php?scheduleid=$scheduleid'><small>join the WAITLIST</small></a></td></tr></table>";
              } else {
                echo "<td width=150><a href='https://www.spirithouse.com.au/booking.php?scheduleid=$scheduleid' title='<b>Click to Book: $class</b><br />$classdesription'>$class</a></td>";
                echo "<td width=100> (".$seatsleft." seats left)</td> ";
                echo "<td width=100>(starts: ".$starttime.")</td></tr></table>";
              }
            }
          } else {
