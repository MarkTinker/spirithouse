...
class Jcart {
	...
	/**
	* Get cart contents
	* This function outputs the items in the cart in each row
	* @return array
	*/
	public function get_contents() {
		...
		foreach($this->items as $tmpItem) {
			...
			$item['subtotalweight'] = $item['weight'] * $item['qty'];
			$items[]          = $item;
		}
		return $items;
	}
	...
	* @return mixed
	*/
	private function add_item($id, $name, $price, $qty = 1, $url, $weight) {
	...
	*/
	private function update_subtotal() {
		...
		/**
	* Recalculate subtotalweight
	*/
	private function update_subtotalweight() {
		$this->itemCount = 0;
		$this->subtotalweight  = 0;

		if(sizeof($this->items > 0)) {
			foreach($this->items as $item) {
				$this->subtotalweight += ($this->qtys[$item] * $this->weights[$item]);
	...
	*/
	...
	public function get_cart_contents() {
		...
		if($this->itemCount > 0) {
			...
			foreach($this->get_contents() as $item)	{
				...
				$cartcontents[$loop]['id']=$item['id'];
				$cartcontents[$loop]['name']=$item['name'];
				$cartcontents[$loop]['qty']=$item['qty'];
				$cartcontents[$loop]['weight']=$item['weight'];
				$cartcontents[$loop]['subtotal']=$item['subtotal'];
				$cartcontents[$loop]['subtotalweight']=$item['subtotalweight'];
	...
	public function display_cart() {
		...
		$postcode="";
		if(isset($_GET['postcode']) && is_numeric($_GET['postcode'])) $postcode=$_GET['postcode'];
		if(isset($_SESSION['postcode']) && is_numeric($_SESSION['postcode'])) $postcode=$_SESSION['postcode'];
		...
		// Add an item
		if ($_POST[$add]) {
			$itemAdded = $this->add_item($id, $name, $price, $qty, $url, $weight);
		...
		if ($isCheckout !== true) {
			...
			if($this->itemCount>0 && !strpos($_SERVER['SCRIPT_NAME'],"shop_checkout.php")) {
				echo tab(7) . "<input type='$inputType' $src id='jcart-checkout' name='jcartCheckout' class='jcart-button' value='Checkout' />\n";
			}
		}
		...
		if($this->itemCount > 0) {
			...
			foreach($this->get_contents() as $item)	{
				...
				if ($item['url']) {
					...
					echo tab(7) . $item['name'];// . " weight ".$item['weight']. "\n"; // displaying weight for testing 
				}
			...
			if(function_exists('calc_shipping')) {
				$shipping=calc_shipping($totalweight,$postcode);
			} else {
				$shipping=0;
			}
			############################################	
			...
			echo tab(4) . "<tr class='jcart-item-subtotal'>\n";
			echo tab(5) . "<td></td>";
			echo tab(5) . "<td>Subtotal:</td>";
			echo tab(6) . "<td class='jcart-item-price'><strong id='jcart-title'>$currencySymbol" . number_format($this->subtotal, $priceFormat['decimals'], $priceFormat['dec_point'], $priceFormat['thousands_sep']) . "\n";
			echo tab(6) . "</td>";
			echo tab(5) . "</tr>". "\n";
			
			if($shipping>0) {

				echo tab(4) . "<tr class='jcart-item-spacer'>\n";
				echo tab(5) . "<td></td>";
				echo tab(5) . "<td></td>";
				echo tab(6) . "<td class='jcart-item-price'><strong id='jcart-title'></td>";
				echo tab(5) . "</tr>". "\n";
					
				echo tab(4) . "<tr class='jcart-item-spacer'>\n";
				echo tab(5) . "<td></td>";
				echo tab(5) . "<td>Shipping to ".$postcode.":</td>";
				echo tab(6) . "<td class='jcart-item-price'><strong id='jcart-title'>$currencySymbol" . number_format($shipping,2) . "\n";
				echo tab(6) . "</td>";
				echo tab(5) . "</tr>". "\n";
				
				if($handling>0) {
					echo tab(4) . "<tr class='jcart-item-spacerl'>\n";
					echo tab(5) . "<td></td>";
					echo tab(5) . "<td>Handling:</td>";
					echo tab(6) . "<td class='jcart-item-price'><strong id='jcart-title'>$currencySymbol" . number_format($handling,2) . "\n";
					echo tab(6) . "</td>";
					echo tab(5) . "</tr>". "\n";					
				}
				...
				echo tab(4) . "<tr class='jcart-item-subtotal'>\n";
				echo tab(5) . "<td></td>";
				echo tab(5) . "<td>Total:</td>";
				echo tab(6) . "<td class='jcart-item-price'><strong id='jcart-title'>$currencySymbol" . number_format(($shipping+$handling+$this->subtotal),2) . "\n";
				echo tab(6) . "</td>";
				echo tab(5) . "</tr>". "\n";
			}
		...
		if(strpos($_SERVER['SCRIPT_NAME'],"shop_checkout.php")!==false) {
			echo "<div style='float:left'><p><a href='shop2.php'>&larr; Continue shopping</a></p></div>";
		}
		  				
		echo tab(3) . "<div id='jcart-buttons' style='float:right'>\n";
		...
		if(strpos($_SERVER['SCRIPT_NAME'], "shop2.php")===false && strpos($_SERVER['SCRIPT_NAME'], "relay.php")===false) {

			if(is_numeric($postcode) && strlen($postcode)==4) {

				echo "<div style='border:1px dotted #999; background-color:#f0f0f0;'><center><h4>Want to calculate shipping for a different postcode?</h4></center>";
				echo "<center><form>Enter your postcode: <input name='postcode' size='10' /> <input type='submit' value='Recalculate shipping >>' /></form></center>";
				echo "</div>";
				
			} else {
			
				echo "<div style='border:1px dotted #999; background-color:#f0f0f0;'><center><h4>calculate shipping &amp; Checkout</h4></center>";
				echo "<center><form>Enter your postcode: <input name='postcode' size='10' /> <input type='submit' value='Calculate and Checkout >>' /></form></center>";
				echo "</div>";
