	...
	if (isset($_POST['search'])) {
		...
		$no=mysql_num_rows($data);
		 ...
		 if ($no){
	            ...
	            $end_result.= "<li><a class='search' href='search-results.php?classid=$classid'>".str_ireplace($word, $bold, $classname)."</a></li>";
	            }
		...
		// we will use this to bold the search word in result
	            $bold= '<span class="found">' . $word . '</span>';
	            $end_result.= "<li><a class='search' href='search-results.php?itemid=$itemid'>".str_ireplace($word, $bold, $result)."</a></li>";
