...
//process_payment(1000, 'testing eway script', 1, 'acland', 'brierty', 'abrierty@gmail.com', 'acland', 4444333322221111, 01, 2016, 123);
//echo "this is a test";

	
function process_payment($total_to_process, $description, $transactionid, $firstname, $lastname, $email, $nameoncard, $cardnumber, $expiry_m, $expiry_y, $ccv) {

  $total_to_process=str_replace("$", "", $total_to_process);
  #$total_to_process=100;
  $total_to_process=$total_to_process*100;

  $eway = new EwayPayment( '13885527', 'https://www.eway.com.au/gateway_cvn/xmlpayment.asp' );
  
  //$eway = new EwayPayment( '87654321', 'https://www.eway.com.au/gateway/xmltest/testpage.asp' ); // TEST GATEWAY

  $eway->setCustomerFirstname( $firstname );
  $eway->setCustomerLastname( $lastname );
  $eway->setCustomerEmail( $email );
  $eway->setCustomerAddress( '' );
  $eway->setCustomerPostcode( '' );
  $eway->setCustomerInvoiceDescription( $description );
  $eway->setCustomerInvoiceRef( $transactionid );
  $eway->setCardHoldersName( $nameoncard );
  $eway->setCardNumber( $cardnumber );
  $eway->setCardExpiryMonth( $expiry_m );
  $eway->setCardExpiryYear( $expiry_y );
  $eway->setTrxnNumber( $transactionid );
  $eway->setTotalAmount( $total_to_process );
  $eway->setCVN( $ccv );

  if( $eway->doPayment() == EWAY_TRANSACTION_OK ) {
      return $eway->getAuthCode();
  } else {
      return $eway->getError().": " . $eway->getErrorMessage();
  }
